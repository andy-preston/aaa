...
Deno.test("A simple javascript expression", () => {
    const instruction = fromTokens(context, ["", "AND", "R12", "0xF * 3"]);
    assertEquals(instruction.mnemonic, "AND");
    assertEquals(instruction.operands, [12, 45]);
});
...
Deno.test("An indexed instruction with an offset", () => {
    const instruction = fromTokens(context, ["", "LDD", "R16", "Z+", "15"]);
    assertEquals(instruction.mnemonic, "LDD");

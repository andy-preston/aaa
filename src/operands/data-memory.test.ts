...
Deno.test("A Data Memory Address is 0 - 0xFFFF", () => {
    ...
    assertThrows(
        ...
        OperandOutOfRange,
        "should be 16 bit Data Memory address (0 - 0xFFFF) (64 K) not -1"
    );
    assertThrows(
        () => operands.numeric("dataAddress16Bit", "0x10000"),
        OperandOutOfRange,
        "should be 16 bit Data Memory address (0 - 0xFFFF) (64 K) not 0x10000"
    );
});
...
Deno.test("A Data Memory Address can't exceed available data memory", () => {
    ...
    assertThrows(
        ...
        OperandOutOfRange,
        "should be within data memory 0 - 0x80 not 0x90"
    );
});
...
Deno.test("A 7 bit Data Memory Address is 0 - 0x7F", () => {
    ...
    assertThrows(
        ...
        OperandOutOfRange,
        "should be 7 bit Data Memory address (0 - 0x7F) (127 Bytes) not -1"
    );
    assertThrows(
        () => operands.numeric("dataAddress7Bit", "0x80"),
        OperandOutOfRange,
        "should be 7 bit Data Memory address (0 - 0x7F) (127 Bytes) not 0x80"
    );
});
